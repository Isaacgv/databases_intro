...
--
select count(distinct o.ship_country) 
from orders o; 
 
select distinct o.ship_country as country 
from orders o
order by country;
...
select c.country,count(c.customer_id) as cnt 
from customers c
group by c.country
order by cnt desc;
...
select * 
from orders 
where lower(ship_country) like 'france' or upper(ship_country) like 'BELGIUM';

--10
select * 
from orders 
where ship_country ilike 'france' or ship_country ilike 'BELGIUM';
--10
select * 
from orders 
...
-- 
select p.product_name, p.discontinued 
from products p
...
from sumPerOrder;
-- 14
select avg( (1-discount) * unit_price * quantity) 
from order_details od;
...
VALUES(11078, 78, 8, 2, 0);
INSERT INTO order_details (order_id, product_id, unit_price, quantity, discount) 
VALUES(11078, 77, 13, 2, 0);
